...
async function loadCollectionFromFS(name, autoInstall = false, scope = "@iconify-json") {
  ...
  async function task() {
    ...
    let jsonPath = localPkg.resolveModule(`${packageName}/icons.json`);
    ...
    if (!jsonPath && !isLegacyExists && autoInstall) {
      ...
      jsonPath = localPkg.resolveModule(`${packageName}/icons.json`);
    }
    if (!jsonPath) {
      const packagePath = localPkg.resolveModule(packageName);
      if (packagePath) {
